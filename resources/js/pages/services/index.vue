...
export default {
    ...
    methods:{
        ...
        update(index){
             this.$router.push( {name:'servicesUpdate', params: { id: this.records[index].id }});
        },
        show(index){
             this.$router.push( {name:'servicesShow' , params: { id: this.records[index].id }});
        },
        async paginate(currentPage){
            this.$router.push({  query: { ...this.$route.query,'page': currentPage }});
        },
        async getRecords(){
            let response = await this.Api('GET','services',this.features());
         ...
         async deleteRecord(index){
            ...
            if(confirm('هل انت متأكد من مسح هذا العنصر')){
                await this.Api('DELETE',`services/${this.records[index].id}`);
