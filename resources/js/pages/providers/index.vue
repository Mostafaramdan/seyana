...
export default {
    ...
    methods:{
        show(index){
             this.$router.push( {name:'providersShow' , params: { id: this.records[index].id }});
        },
        create(){
             this.$router.push( {name:'providersCreate'});
        },
        update(index){
             this.$router.push( {name:'providersUpdate', params: { id: this.records[index].id }});
        },
        ...
        async getRecords(){
            ...
            let response = await this.Api('GET','providers',this.features());
         ...
         async deleteRecord(index){
            ...
            if(confirm('هل انت متأكد من مسح هذا العنصر')){
                await this.Api('DELETE',`providers/${this.records[index].id}`);
    ...
    metaInfo() {
        return {
            title: `${this.$store.state.appName} -   مزودي الخدمات `,
        }
    },
    watch :{
        search:function(val){
